...
void loop()
{
  ...
  // LED indicator
  if (distance > 0 && distance < 0.05) // 5 cm
  {
    digitalWrite(7, HIGH);
    delay(50);
  }
  else if (distance >= 0.05 && distance < 0.1) // 10 cm
  {
    digitalWrite(7, HIGH);
    delay(50);
    digitalWrite(7, LOW);
  ...
  else if (distance >= 0.1 && distance < 0.2) // 20 cm
  {
    digitalWrite(7, HIGH);
    delay(210);
    digitalWrite(7, LOW);
  ...
  else if (distance >= 0.2 && distance < 0.35) // 35 cm
  {
    digitalWrite(7, HIGH);
    delay(610);
    digitalWrite(7, LOW);
...
float ultrasonicRead()
{
  ...
  lcd.setCursor(0, 0);
  lcd.print("d m:    ");
  lcd.print(distance, 2); // Display with 2 decimal places
  lcd.print("   ");
  return distance;
}
